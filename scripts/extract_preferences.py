...
def write_review_file(clusters: list[dict], output_path: Path, stats: dict) -> None:
    ...
    lines = [
        ...
        f"**Pass stability threshold:** {len(clusters)}",
        "",
        "## How to use this file",
        "",
        "For each candidate below, decide: ACCEPT, EDIT, or REJECT.",
        "",
        ...
        "- **REJECT**: just leave it here; the file will be archived",
        "",
        "Once you've moved approved candidates, run `python scripts/refresh_indexes.py` and delete this file.",
        "",
        "---",
        "",
    ]
    ...
    if not clusters:
        lines.append("_No candidates passed stability thresholds. Try lowering `--min-count` or scanning more sessions._")
    else:
...
def main():
    ...
    write_review_file(stable, output_path, stats)
    print()
    print(f"Review file: {output_path}")
    print(f"Open it, decide ACCEPT/EDIT/REJECT for each candidate.")
