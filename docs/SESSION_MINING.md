...
Your Claude Code sessions are stored at `~/.claude/projects/<project>/<session-uuid>.jsonl`. Every time you've corrected Claude ("no, do it this way"), expressed a preference ("I prefer X"), or taught a lesson ("we got burned by Y") — those signals are in your history.
...
### Mode A — One-time bootstrap (`/moradin:learn-from-sessions`)
...
### Mode B — Per-session incremental (via `/moradin:ship`)

After each session ends, `/moradin:ship` can process just that session's turns and add to a candidate pool. Cross-session aggregation happens later via `/moradin:retrospect`.
...
~/.claude/projects/<project>/*.jsonl
     ↓ session_scanner.py — parse + redact + flatten
high-signal user turns (after regex pre-filter)
     ↓ Haiku (cheap classifier) — keep correction/preference/lesson, drop noise
filtered candidates
     ↓ Sonnet (strong extractor) — structured output per turn
extracted candidates ({title, statement, category, confidence, source})
     ↓ Cluster (substring + token overlap)
candidate clusters with occurrence counts
     ↓ Stability filter (count ≥ 5, sessions ≥ 3, confidence ≥ 0.6)
review queue → scratch/proposed_<date>.md
     ↓ Operator review (ACCEPT / EDIT / REJECT each)
...
| `--min-sessions N` | 3 | Pattern must span ≥ N distinct sessions. Filters one-off bursts. |
...
- 10k turns of 180d history → ~$8-15
...
| Secrets in sessions | Regex redaction strips Anthropic/OpenAI/GitHub tokens, emails, bearer tokens before sending to LLM. Not exhaustive — review redaction output if needed. |
...
- Don't aim for 100% accuracy — aim for "useful." Even 10 well-captured preferences is a strong starting workshop layer.
...
- Cluster matching is simple substring + token overlap (not embeddings) — may not group semantically-equivalent-but-differently-worded candidates. Embedding-based clustering is a Phase 2+ enhancement.
- Single-session corrections won't surface (deliberately — see stability thresholds)
